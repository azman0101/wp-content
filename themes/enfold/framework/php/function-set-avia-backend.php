...
{
	/**
	 * Converts a hex string into an rgb array. Accepts 3 or 6 characters.
	...
	function avia_backend_hex_to_rgb_array( $color )
	{
		...
		$color = ( strlen( $color ) <= 3 ) ? str_split( $color, 1 ) : str_split( $color, 2 );
		
		for( $key = 0; $key < 3; $key++ )
		{
			if( isset( $color[ $key ] ) && ( strlen( $color[ $key ] ) >= 1 ) )
			{
				if( 1 == strlen( $color[ $key ] ) )
				{
					$color[ $key ] .= $color[ $key ];
				}
				$color[ $key ] = hexdec( $color[ $key ] );
			}
			else
			{
				//	fallback to black as default
				$color[ $key ] = 0;
			}
		}
