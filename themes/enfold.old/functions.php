...
wp_register_script('clicrdv_script', '//clicrdv-assets.s3.amazonaws.com/lib/clicrdv-widgets-v1/clicrdv-widgets-min.js');
wp_enqueue_script('clicrdv_script');
}

add_action( 'wp_enqueue_scripts', 'wpb_adding_scripts' );


# register mixpanel script
#wp_register_script( 'mixpanel', get_template_directory_uri() . '/js/custom.js');

# enqueue mixpanel
#wp_enqueue_script('mixpanel');


// wp_deregister_script('mixpanel_tracker');
// Register the script
#wp_register_script( 'mixpanel_tracker', get_template_directory_uri() . '/js/tracker.js');
...
function show_page_name($wp){
     if ( ! is_admin() ){
        global $post;   
        register_page_title_mixpanel($post->post_name);
     }
}

function register_page_title_mixpanel($title) {
// Localize the script with new data
$metadata_array = array(
	'type' => '',
	'title' => $title
);
#wp_localize_script( 'mixpanel_tracker', 'ObjPHP', $metadata_array );

// Enqueued script with localized data.
#wp_enqueue_script( 'mixpanel_tracker', array('mixpanel'));
